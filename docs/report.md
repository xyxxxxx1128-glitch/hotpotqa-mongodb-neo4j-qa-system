...
本项目使用 HotpotQA 数据集。HotpotQA 不是数据库，而是多跳问答数据集。数据集来源为 Hugging Face：

```text
https://huggingface.co/datasets/hotpotqa/hotpot_qa
```

本项目使用其中的 `distractor` 配置。该配置原始规模约为：
...
实验版本从 `train-00000-of-00002.parquet` 中导入前 10000 条真实数据。
...
免费云数据库容量有限，不适合完整承载 HotpotQA 全量数据。本地全量导入也会占用较多磁盘空间和导入时间。课程作业的重点是验证数据库建模、查询、聚类和可视化流程，因此本项目选取 10000 条真实 HotpotQA 数据作为实验规模。

选择 10000 条的原因：

- 数据量明显大于简单样例，能够形成较大规模图谱
- 可以展示关键词检索、多跳查询、聚类统计和可视化功能
- 导入时间和本地磁盘占用仍然可控
- 导入脚本支持通过 `--limit` 调整规模，也支持不设置 `--limit` 时导入完整 split
